...
// POST { target_type: 'user'|'company', target_id | target_uuid }
// Creates a follow from the CURRENT ACTOR (user or company session)
// to the target. Both users and companies can follow.
// =====================================================================
...
// Can't follow yourself (same identity, same type).
...
// This is a USER preference; company sessions have no such setting.
...
$stmt = $pdo->prepare(
    'INSERT IGNORE INTO follows (follower_type, follower_id, target_type, target_id)
     VALUES (?, ?, ?, ?)'
);
...
if (!$alreadyFollowing) {
    Social::notify($targetType, $targetId, $actor['type'], $actor['id'], 'follow');
}
